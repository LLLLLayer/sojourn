...
Sojourn 是一个从 AI 编码对话中自动提炼可复用知识的团队工具。读取 Claude Code、opencode 等对话日志，通过 AI 分析重建为思维树、SOP、工作流模式三种结构化知识，输出到 CLAUDE.md、Git 仓库、Memory 等可插拔 sink。
...
pnpm build          # 构建所有包
pnpm lint           # ESLint 检查
pnpm test           # 运行测试（vitest, 20 tests）
```
...
- **`packages/web`** — React 前端（会话卡片网格、对话预览抽屉、思维树/SOP/工作流可视化、暂存管理、设置页）
